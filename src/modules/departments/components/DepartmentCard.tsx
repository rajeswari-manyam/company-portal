...
function DepartmentCard({ department, index, onEdit, onDelete, onView }: DepartmentCardProps) {
  ...
  return (
    <div
      className="group relative bg-white rounded-2xl border border-slate-100 p-6 flex flex-col gap-4
                 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden cursor-pointer"
    ...
    >
        ...
        <div className={`flex items-center gap-1.5 transition-all duration-200 ${hovered ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-1'}`}>
          <button
            ...
            className="p-2 rounded-lg bg-gradient-to-r from-[#0B0E92] to-[#69A6F0]
                       text-white shadow-sm hover:shadow-md hover:scale-105 transition-all duration-150"
          >
            <Edit2 size={13} />
          </button>
          <button
      ...
      <div>
        ...
        {department.description && (
          <p className="text-sm text-slate-500 break-words line-clamp-3">
            {department.description}
          </p>
        )}
      </div>
