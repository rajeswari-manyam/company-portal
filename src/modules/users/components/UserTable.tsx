          ...
          <div className="flex gap-1 border-b border-slate-100">
            {(['Personal', 'Job'] as ProfileTab[]).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-all duration-200 ${tab === t ? 'bg-gradient-to-r from-[#0B0E92] to-[#69A6F0] text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'
                  }`}>{t}</button>
            ))}
          </div>
        </div>
      ...
      <div className="flex items-center gap-1">
        ...
        {pages.map((p, idx) => p === 'ellipsis'
          ...
          : <button key={p} onClick={() => onPageChange(p)}
            className={`min-w-[30px] h-[30px] rounded-lg text-xs font-medium transition-all duration-200 ${p === page ? 'bg-gradient-to-r from-[#0B0E92] to-[#69A6F0] text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100'
              }`}>{p}</button>
        )}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-100">
              {/* ✅ Removed: Designation, Role columns */}
              {['Employee', 'ID', 'Department', 'Status', 'Actions'].map(h => (
