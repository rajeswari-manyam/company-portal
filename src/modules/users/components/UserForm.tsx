...
// ✅ Fixes:
//   1. DepartmentRecord now imported from '../../service/Department.service' (correct path)
//   2. Explicit DepartmentRecord type on departments.map(d => ...) — no more implicit any
...
import {
  getDepartments,
  type DepartmentRecord,           // ✅ imported from correct service path
...
export default function UserForm({ initial, onSuccess, onCancel }: UserFormProps) {
    ...
    getDepartments()
      .then(res => {
        if (res.success && res.departments.length > 0) {
          ...
          // ✅ Fix: explicit type on d — no more implicit any
          console.table(res.departments.map((d: DepartmentRecord) => ({ ID: d._id, Name: d.departmentName })));
          console.groupEnd();
        } else {
  ...
  // ── Log form state on every change ──────────────────────────────────────────
  useEffect(() => {
    console.group('%c📝 [Form State]', 'color:#0ea5e9;font-weight:bold');
    console.table(form);
    console.groupEnd();
  }, [form]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.departmentId) { setApiError('Please select a department.'); return; }

    setSubmitting(true);
    try {
      if (isEdit && initial?._id) {
        const payload: UpdateEmployeePayload = {
          name: form.name, email: form.email, phone: form.phone, empId: form.empId,
        ...
        console.log('_id:', initial._id); console.table(payload); console.groupEnd();
        ...
        const payload: CreateHRPayload = {
          name: form.name, email: form.email, phone: form.phone, hrId: form.empId,
          departmentId: form.departmentId, designation: form.designation,
          gender: form.gender, dateOfBirth: form.dateOfBirth,
          dateOfJoining: form.dateOfJoining, address: form.address,
        };
        console.group('%c🧑‍💼 [CREATE HR]', 'color:#8b5cf6;font-weight:bold;font-size:13px');
        console.table(payload); console.groupEnd();
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <TextInput label="Full Name" icon={<User size={12} />} value={form.name} onChange={set('name')}
            placeholder="e.g. Rahul Sharma" required wrapperClassName="sm:col-span-2" />
          <TextInput label="Email Address" icon={<Mail size={12} />} type="email" value={form.email}
            onChange={set('email')} placeholder="rahul@company.com" required />
          <TextInput label="Phone Number" icon={<Phone size={12} />} type="tel" value={form.phone}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <SelectField label="Role" icon={<Shield size={12} />} value={form.role} onChange={set('role')}>
            {ROLE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </SelectField>
          <TextInput label={form.role === 'hr' ? 'HR ID' : 'Employee ID'} icon={<Hash size={12} />}
            value={form.empId} onChange={set('empId')}
            placeholder={form.role === 'hr' ? 'HR001' : 'EMP001'} required />
            ...
            {deptLoading ? (
              ...
              <div className="relative">
                <select value={form.departmentId} onChange={set('departmentId')} required className={selectBase}>
                  ...
                  {/* ✅ Explicit type: DepartmentRecord */}
          ...
          <TextInput label="Designation" icon={<Briefcase size={12} />} value={form.designation}
            onChange={set('designation')} placeholder="e.g. Software Engineer" required />
          <TextInput label="Date of Joining" icon={<Calendar size={12} />} type="date"
            value={form.dateOfJoining} onChange={set('dateOfJoining')} wrapperClassName="sm:col-span-2" />
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <TextInput label="Date of Birth" icon={<Calendar size={12} />} type="date"
      ...
      <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3 pt-2 border-t border-slate-100">
        ...
        <button type="submit" disabled={submitting}
          ...
          {submitting ? 'Saving…' : isEdit ? 'Save Changes' : form.role === 'hr' ? 'Create HR' : 'Create Employee'}
        </button>
      </div>
    </form>
  );
