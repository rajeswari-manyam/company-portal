  ...
  const lookup = useCallback((
    ...
    timer.current = setTimeout(async () => {
      try {
        ...
        if (match) {
          onFound({ name: match.name ?? '', email: match.email ?? '', department: match.department ?? '' });
          setFound(true);
        } else {
          onNotFound();
...
export default function AdminPayroll() {
  ...
  const resetAdd = () => { setEmpId(''); setName(''); setEmail(''); setDept(''); setSalary(''); addLookup.reset(); };
  const resetInc = () => { setIncId(''); setIncName(''); setIncEmail(''); setIncDept(''); setIncAmt(''); incLookup.reset(); };

  const handleAddEmployee = async () => {
    if (!empId || !name || !email || !dept || !salary) return alert('Please fill all fields');
    setSubmitting(true);
    try {
  ...
  const handleIncrement = async () => {
    ...
    } catch { alert('Failed to apply increment.'); }
    finally { setSubmitting(false); }
  };
    ...
    >
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 font-normal">Generate and manage employee payslips</p>
        </div>
        ...
        <Modal title="Add Employee" onClose={() => { setShowAdd(false); resetAdd(); }}>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Full Name"   placeholder={addLookup.loading ? 'Looking up…' : 'Auto-filled'}
                value={name}  readOnly={addLookup.found} onChange={e => setName(e.target.value)}
                loading={addLookup.loading} />
              <Field label="Department" placeholder={addLookup.loading ? 'Looking up…' : 'Auto-filled'}
                value={dept}  readOnly={addLookup.found} onChange={e => setDept(e.target.value)}
                loading={addLookup.loading} />
            </div>
            <Field label="Email" type="email" placeholder={addLookup.loading ? 'Looking up…' : 'Auto-filled'}
              value={email} readOnly={addLookup.found} onChange={e => setEmail(e.target.value)}
              loading={addLookup.loading} />
            <Field label="Base Salary" type="number" placeholder="e.g. 450000"
      ...
      {showIncrement && (
        <Modal title="Increment Salary" onClose={() => { setShowIncrement(false); resetInc(); }}>
          <div className="space-y-3">
            ...
            />
            <div className="grid grid-cols-2 gap-3">
              <Field label="Name"       placeholder={incLookup.loading ? 'Looking up…' : 'Auto-filled'}
                value={incName} readOnly loading={incLookup.loading} />
              <Field label="Department" placeholder={incLookup.loading ? 'Looking up…' : 'Auto-filled'}
                value={incDept} readOnly loading={incLookup.loading} />
            </div>
            <Field label="Email" type="email" placeholder={incLookup.loading ? 'Looking up…' : 'Auto-filled'}
              value={incEmail} readOnly loading={incLookup.loading} />
            <Field label="Increment Amount" type="number" placeholder="e.g. 5000"
              value={incAmt} onChange={e => setIncAmt(e.target.value)} />
          </div>
          <div className="flex gap-3 pt-1">
