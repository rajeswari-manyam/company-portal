...
export default function HRLeaves() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Leave Requests" subtitle="Review and manage employee leave requests" />
      ...
      <Card padding={false}>
        ...
        <LeaveTable leaves={filtered} showEmployee canApprove onApprove={l => approve(l.id, user?.name ?? 'HR')} onReject={l => { setRejecting(l); setRejectNote(''); }} />
      </Card>

      {rejecting && (
        <Modal title="Reject Leave Request" onClose={() => setRejecting(null)}>
          <p className="text-sm text-slate-600 mb-4">Rejecting leave for <strong>{rejecting.userName}</strong> ({rejecting.leaveType}, {rejecting.days} days)</p>
          <Textarea label="Reason (optional)" value={rejectNote} onChange={e => setRejectNote(e.target.value)} placeholder="Provide a reason…" rows={3} />
          <div className="flex justify-end gap-3 mt-4">
            <Button variant="outline" onClick={() => setRejecting(null)}>Cancel</Button>
            <Button variant="danger" onClick={() => { reject(rejecting.id, user?.name ?? 'HR', rejectNote); setRejecting(null); }}>Reject Leave</Button>
          </div>
        </Modal>
      )}
    </div>
