...
// ── CallEntry type (previously in store.ts) ───────────────────────────────────
...
function getCallEntriesForUser(userId: string): CallEntry[] {
  try {
    const raw = localStorage.getItem(STORE_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    return (parsed.callEntries ?? [])
      .filter((c: CallEntry) => c.userId === userId)
      .sort((a: CallEntry, b: CallEntry) => b.createdAt.localeCompare(a.createdAt));
  } catch {
    return [];
  }
}
